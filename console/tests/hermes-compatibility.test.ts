...
describe("Hermes compatibility baseline", () => {
  ...
  it("prepares, activates, and rolls back an isolated runtime manifest", async () => {
    ...
    const oldRuntime = path.join(root, "hermes-v017");
    const newRuntime = path.join(root, "hermes-v020");
    await fsp.writeFile(oldRuntime, "old");
    await fsp.writeFile(newRuntime, "new");
    await prepareHermesRuntime(manifestPath, profileHome, oldRuntime);
    expect((await readHermesRuntimeManifest(manifestPath, profileHome)).candidate_executable).toBe(oldRuntime);
    await activateHermesRuntime(manifestPath, profileHome);
    await prepareHermesRuntime(manifestPath, profileHome, newRuntime);
    await activateHermesRuntime(manifestPath, profileHome);
    const rolledBack = await rollbackHermesRuntime(manifestPath, profileHome);
    expect(rolledBack.active_executable).toBe(oldRuntime);
